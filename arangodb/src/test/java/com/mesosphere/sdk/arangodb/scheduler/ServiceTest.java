package com.mesosphere.sdk.arangodb.scheduler;
...
public class ServiceTest {
    ...
    public void testSpec() throws Exception {
        ...
        ticks.add(Send.offerBuilder("arangodb").build());
        ticks.add(Expect.launchedTasks("arangodb-0-node"));
        ticks.add(Send.taskStatus("arangodb-0-node", Protos.TaskState.TASK_ERROR).build());
        ...
        ticks.add(Send.offerBuilder("arangodb").setPodIndexToReoffer(0).build());
        ticks.add(Expect.launchedTasks("arangodb-0-node"));
        ticks.add(Send.taskStatus("arangodb-0-node", Protos.TaskState.TASK_RUNNING).build());
