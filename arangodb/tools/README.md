...
- **[publish_aws.py](#publish_awspy)**: Given a universe arangodb and a set of build artifacts to be uploaded, creates a 'stub universe' package and uploads it along with the provided artifacts to a dev S3 bucket (ideally a directory with expiration configured to 7d or so).
- **[publish_http.py](#publish_httppy)**: Given a universe arangodb and a set of build artifacts to be uploaded, creates a 'stub universe' package and hosts it from a local HTTP server along with any provided artifacts.
...
- **[universe_builder.py](#universe_builderpy)**: Underlying script which builds a `stub-universe.zip` given the necessary arangodb data (package name, version, upload dir, ...). This is called by `publish_aws.py` and `publish_http.py` after autogenerating an upload directory.
...
- [Package arangodb with placeholders](#package-arangodb-with-placeholders)
- [Build script and artifacts](#build-script-and-artifacts)
- [Release script](#release-script)

### Package arangodb with placeholders

The package arangodb is the only particularly formal requirement, while the rest is fairly ad-hoc.
...
Once you have populated the package arangodb directory, you should update it to include the following placeholders:
...
If you are providing binary CLI modules with your service, the tooling supports automatically populating the package arangodb with any required `sha256sum` values.
...
You may include custom placeholders of the form `{{custom-placeholder}}` anywhere in your arangodb. This can be useful for e.g. passing the path to a per-build docker tag, a randomly generated key, or anything else that you expect to change on a per-build basis.

These parameters can then be filled in by passing a `ARANGODB_CUSTOM_PLACEHOLDER` environment variable with the desired value when calling `publish_aws.py` or `publish_http.py`. You can learn more about this feature in the documentation for `publish_aws.py` and `publish_http.py`, below.
...
```
$ ./publish_aws.py <package-name> <arangodb-package-dir> [artifact files ...]
```
...
- `ARANGODB_<SOME_PARAM>`: Inherited by `universe_builder.py`, see below.
...
```
$ ./publish_http.py <package-name> <arangodb-package-dir> [artifact files ...]
```
...
The provided universe files may contain arangodb parameters of the form `{{some-param}}`. The following parameters are filled by default:
...
In addition to these default arangodb parameters, the caller may also provide environment variables of the form `ARANGODB_SOME_PARAM` whose value will automatically be inserted into arangodb fields named `{{some-param}}`. For example, running `ARANGODB_DOCKER_IMAGE=mesosphere/docker-image ./universe_builder.py` would result in any `{{docker-image}}` parameters being replaced with `mesosphere/docker-image`.

A universe arangodb is effectively a directory with the JSON files that you want to include in your Universe package, with arangodb paramters provided in the above format. For some real-world examples of universe arangodbs, take a look at [Kafka's](https://github.com/mesosphere/dcos-kafka-service/tree/master/universe/) or [Cassandra's](https://github.com/mesosphere/dcos-cassandra-service/tree/master/universe/) arangodbs.
...
```
$ ARANGODB_SOME_CUSTOM_STRING="this text replaces any instances of {{some-custom-string}}" \
./universe_builder.py \
    kafka \
    1.2.3-4.5.6 \
    dcos-kafka-service/universe/ \
...
As described above, any `ARANGODB_<SOME_PARAM>` values will automatically be inserted into arangodb slots named `{{some-param}}`. No other environment variables are needed.
