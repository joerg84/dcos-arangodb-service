...
def print_help(argv):
    logger.info('Syntax: {} <package-name> <arangodb-package-dir> [artifact files ...]'.format(argv[0]))
    ...
    logger.info('In addition, environment variables named \'ARANGODB_SOME_PARAMETER\' will be inserted against the provided package arangodb (with params of the form \'{{some-parameter}}\')')
...
def main(argv):
    ...
    # the package name:
    package_name = argv[1]
    # local path where the package arangodb is located:
