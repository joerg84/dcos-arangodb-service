...
def test_arangodb_service_(mocker):
    ...
    package = Package("arangodb", "stub-universe")
    package_manager = mocker.Mock()

    package_manager.get_latest = mocker.MagicMock(return_value=Package.from_json(package_json))

    upb = UniversePackageBuilder(package, package_manager, 'resources/arangodb', ',', [])

    arangodb_mapping = upb._get_arangodb_mapping_for_content("")
    assert 'upgrades-from' in arangodb_mapping
    assert arangodb_mapping['upgrades-from'] == "1.2.3"
