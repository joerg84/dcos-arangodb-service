...
class UniversePackageBuilder(object):
    ...
    def _get_arangodb_mapping_for_content(self, orig_content):
        '''Returns a arangodb mapping (dict) for the following cases:
        ...
        - Custom environment params like 'ARANGODB_SOME_PARAM' which maps to '{{some-param}}'
        '''
        # default arangodb values (may be overridden via eg ARANGODB_PACKAGE_VERSION envvars):
        now = time.time()
        arangodb_mapping = {
        ...
        # look for any 'sha256:filename' arangodb params, and get shas for those.
        # this avoids calculating shas unless they're requested by the arangodb.
        for shafilename in re.findall('{{sha256:(.+?)}}', orig_content):
            ...
            arangodb_mapping['sha256:{}'.format(
                shafilename)] = self._calculate_sha256(shafilepath)

        # import any custom ARANGODB_SOME_PARAM environment variables:
        for env_key, env_val in os.environ.items():
            if env_key.startswith('ARANGODB_'):
                # 'ARANGODB_SOME_KEY' => 'some-key'
                arangodb_mapping[env_key[len('ARANGODB_'):].lower().replace(
                    '_', '-')] = env_val

        return arangodb_mapping

    def _apply_templating_to_file(self, filename, orig_content):
        arangodb_mapping = self._get_arangodb_mapping_for_content(orig_content)
        new_content = orig_content
        for arangodb_key, arangodb_val in arangodb_mapping.items():
            new_content = new_content.replace('{{%s}}' % arangodb_key,
                                              arangodb_val)
        ...
        arangodb_keys = list(arangodb_mapping.keys())
        arangodb_keys.sort()
        for key in arangodb_keys:
            logger.info('  {{%s}} => %s' % (key, arangodb_mapping[key]))
